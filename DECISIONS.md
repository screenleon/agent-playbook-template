...
## 2026-04-11: Agent-deference principle, trust levels, and automation support

- **Context**: The template applied heavy ceremony (4 mandatory + 2 recommended human checkpoints) to all tasks regardless of risk. This made Small/Medium tasks disproportionately slow, especially when the underlying agent tool (Copilot, Claude Code, Cursor, etc.) already provides safety rails, tool routing, and validation natively. Additionally, other agent frameworks (Claude Code's permission modes, devin.cursorrules autonomous planning, Aider's auto-lint/auto-test/auto-commit) demonstrate that significant automation is both safe and effective when guardrails are properly scoped. Some users want fully unattended execution and, as long as they explicitly accept the risk, the template should not impose a hard floor.
- **Decision**: Three changes: (1) **Agent-deference principle** — the template only adds rules the agent tool does not already provide natively. Agent-native items are marked `[AGENT-NATIVE]` and can be trimmed during adoption. (2) **Trust level mechanism** — three tiers (`supervised`, `semi-auto` default, `autonomous`) plus an opt-in `dangerouslySkipAllCheckpoints: true` flag. Activation matrix is 6 gates × 4 columns. Always-safe operations need no approval at any level. By default, always-dangerous operations require approval even at `autonomous`; with `dangerouslySkipAllCheckpoints: true` (requires `trust_level: autonomous`), all gates including destructive actions are bypassed. (3) **Autonomous validation loop** — the test-fix-repeat cycle runs without human approval between iterations; escalation after 3 consecutive failures (skipped in bypass mode).
- **Alternatives considered**: Hard floor on destructive actions regardless of user choice — rejected because if a user explicitly opts in knowing the risks, the template has no right to override that choice; the safeguard is the explicit double opt-in (both `autonomous` + `dangerouslySkipAllCheckpoints`) and the recommended preconditions. Single binary mode — rejected because most projects still need a middle ground. Per-file trust — rejected as too granular.
- **Constraints introduced**: `dangerouslySkipAllCheckpoints` has no effect without `trust_level: autonomous`. It does not override security rules (no secrets in code, no credential exposure). Activation matrix must be consulted for gate decisions. Validation loop is autonomous at all trust levels.

## 2026-04-10: Rule identity and supersession integrity enforcement

- **Context**: Initial rule governance automation validated structure, but did not enforce unique Rule IDs or explicit supersession links, leaving room for ambiguous references and stale rule states.
- **Decision**: Enhanced `scripts/lint-layered-rules.sh` to enforce unique Rule IDs and validate supersession references; expanded domain templates with `Superseded by` field to support deterministic replacement tracking.
- **Alternatives considered**: Keep these checks manual in PR review. Rejected because ambiguity is easy to miss and expensive to debug later.
- **Constraints introduced**: Rule IDs must be unique across layered rule files. Rules marked `Status: superseded` must specify a valid replacement rule via `Superseded by`.

## 2026-04-10: Rule governance execution (domain templates + CI lint)

- **Context**: The optimization roadmap required practical adoption assets and automation; without templates and checks, layered governance would remain manual and drift-prone.
- **Decision**: Implemented Phase 2 and initial Phase 3 items: domain rule templates, project override annotation format, PR checklist, and CI lint workflow for layered rules.
- **Alternatives considered**: Keep optimization as documentation-only guidance. Rejected because it does not prevent regressions or ensure consistent rule authoring.
- **Constraints introduced**: Rule changes should pass `scripts/lint-layered-rules.sh` and include override annotations when project rules supersede base rules.

## 2026-04-10: Two-step rule loading (quickstart first, deep docs on demand)

- **Context**: Rule coverage is strong, but the main rule set has grown large enough to increase first-pass reading cost for agents and maintainers.
- **Decision**: Introduced `docs/rules-quickstart.md` as the first-pass loading file and `docs/rule-optimization-plan.md` as the staged improvement roadmap, while keeping source-of-truth details in existing deep docs.
- **Alternatives considered**: Move all details into one short file. Rejected because it would either lose critical nuance or reintroduce growth and drift in the short file over time.
- **Constraints introduced**: Quickstart must stay compact and reference canonical docs instead of duplicating full policy prose. New rule additions should update quickstart only when they affect first-pass decision making.

## 2026-04-10: Layered configuration governance model (placement + deterministic resolution)

- **Context**: After introducing layered configuration, precedence alone was still insufficient for maintainability. Teams could still misplace rules, create same-layer conflicts, and accidentally shadow constraints by duplication.
- **Decision**: Added governance model with three controls: (1) rule placement rubric (lowest safe ownership layer), (2) deterministic resolution algorithm for cross-layer and same-layer conflicts, and (3) hygiene guardrails to prevent duplicate rule text and stale overrides.
- **Alternatives considered**: Keep precedence-only guidance and rely on reviewer judgment. Rejected because conflict handling remains subjective and inconsistent across teams.
- **Constraints introduced**: If layer ownership is unclear, default to Domain Rules (not Global). Same-layer tie-break uses narrower scope first, then most recent dated rule, with tie-break captured in `DECISIONS.md`.

## 2026-04-10: Layered configuration, initialization protocol, and architecture-doc/test-first hardening

- **Context**: The template already had strong workflow controls, but improvement feedback highlighted maintainability risks: constraints were still perceived as one large bundle, first-session boundary discovery was not explicit enough, and architecture/test rigor needed more enforceable wording for behavior-changing work.
- **Decision**: Adopted three concrete changes: (1) layered configuration scaffolding with `rules/global/`, `rules/domain/`, and `project/project-manifest.md`; (2) first-entry initialization protocol via `skills/on_project_start/SKILL.md`; (3) mandatory TDAI test-first requirement for new behavior plus ADR automatic update requirement for architecture changes.
- **Alternatives considered**: Keep all rules inside existing source files only and rely on prose guidance. Rejected because constraints remain hard to adapt per project and easy to overlook during first-entry sessions.
- **Constraints introduced**: Conflict precedence is now explicit (user instruction -> existing repo practice -> project context -> domain -> global). Behavior-changing work should define tests before implementation except trivial non-behavioral edits. Architecture changes must update ADRs (or decision log fallback) in the same task.
