...
- `prompt-budget.yml` (at repo root) declares execution mode (`supervised`, `semi-auto`, or `autonomous`), enabled roles, token budget, and optional abstract model-tier routing policy. Keep concrete provider/model mapping in adapter or local runtime config. See `docs/adoption-guide.md` → Autonomous execution mode for when and how to use autonomous mode.
