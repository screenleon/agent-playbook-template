...
9. Record decisions and ADR updates when architecture changes. **Exception**: if `prompt-budget.yml` → `decision_log.policy: example_only`, record in the task summary or trace instead of appending to `DECISIONS.md`.
