#!/usr/bin/env bash
# harness/adapters/windsurf/conformance.sh
# ─────────────────────────────────────────────────────────────────────────────
# Conformance check for the windsurf adapter.
...
printf '=== windsurf adapter conformance ===\n'
...
# windsurf-specific: harness-rules.md must be present
harness_rules="$ROOT_DIR/harness/adapters/windsurf/harness-rules.md"
if [[ -f "$harness_rules" ]]; then
  if grep -q "execution_mode" "$harness_rules"; then
    printf 'PASS  harness-rules.md references execution_mode\n'
    (( pass_count++ )) || true
  else
    printf 'FAIL  harness-rules.md does not reference execution_mode\n'
    (( fail_count++ )) || true
  fi
else
  printf 'FAIL  harness-rules.md not found (required for windsurf adapter)\n'
  (( fail_count++ )) || true
fi

adapter_doc="$ROOT_DIR/harness/adapters/windsurf/ADAPTER.md"
