#!/usr/bin/env bash
# harness/adapters/copilot/conformance.sh
# ─────────────────────────────────────────────────────────────────────────────
# Conformance check for the copilot adapter.
...
printf '=== copilot adapter conformance ===\n'
...
# Copilot-specific: governance-block.md must be present
governance_doc="$ROOT_DIR/harness/adapters/copilot/governance-block.md"
if [[ -f "$governance_doc" ]]; then
  if grep -q "execution_mode" "$governance_doc"; then
    printf 'PASS  governance-block.md references execution_mode\n'
    (( pass_count++ )) || true
  else
    printf 'FAIL  governance-block.md does not reference execution_mode\n'
    (( fail_count++ )) || true
  fi
else
  printf 'FAIL  governance-block.md not found (required for copilot adapter)\n'
  (( fail_count++ )) || true
fi

adapter_doc="$ROOT_DIR/harness/adapters/copilot/ADAPTER.md"
