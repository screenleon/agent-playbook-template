#!/usr/bin/env bash
# harness/adapters/cursor/conformance.sh
# ─────────────────────────────────────────────────────────────────────────────
# Conformance check for the cursor adapter.
...
printf '=== cursor adapter conformance ===\n'
...
# Cursor-specific: harness.mdc must be present
harness_mdc="$ROOT_DIR/harness/adapters/cursor/harness.mdc"
if [[ -f "$harness_mdc" ]]; then
  if grep -q "execution_mode" "$harness_mdc"; then
    printf 'PASS  harness.mdc references execution_mode\n'
    (( pass_count++ )) || true
  else
    printf 'FAIL  harness.mdc does not reference execution_mode\n'
    (( fail_count++ )) || true
  fi
else
  printf 'FAIL  harness.mdc not found (required for cursor adapter)\n'
  (( fail_count++ )) || true
fi

adapter_doc="$ROOT_DIR/harness/adapters/cursor/ADAPTER.md"
